...
class TrueFalseComboBox(ttk.Combobox):
    def __init__(self, *args):
        super().__init__(*args, values = ["True", "False"])
...
def run_func(func_to_run: Callable) -> None:
    ...
    for i in range(len(params)):
        ...
        entries.append(type_of_widget(window).place(relx=width, rely=offset*i, relwidth=1-width, relheight=offset, anchor="nw"))
    try: i
    except: return
...
if __name__ == '__main__':
    ...
    run_func(InstagramDataVisualizer.visualize_most_active_year)
