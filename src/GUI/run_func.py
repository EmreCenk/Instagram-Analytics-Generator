...
class LiteralComboBox(ttk.Combobox):
    def __init__(self, *args, options=[]):
        super().__init__(*args, values=options, state = "readonly")
...
def run_func_via_gui(func_to_run: Callable, ready_inputs=None) -> None:
    ...
    def execute():
        ...
        for i in range(len(args)):
            ...
            elif hasattr(types[i], "__origin__") and types[i].__origin__ is Literal: # Using Litera["a", "b"] as an input type for more descriptiveness
                print("ya")
    ...
    for i in range(len(params)):
        ...
        if hasattr(types[i], "__origin__") and types[i].__origin__ is Literal: # Using Litera["a", "b"] as an input type for more descriptiveness
            literal_values = set(types[i].__args__)
            current = LiteralComboBox(window, options=sorted(literal_values, key=lambda x: -len(x)))
        elif params[i] == "interval": current = IntervalComboBox(window)
